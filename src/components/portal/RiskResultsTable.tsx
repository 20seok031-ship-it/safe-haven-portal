...
export default function RiskResultsTable({ results, formInfo, uploadedImages, onUpdateResult }: RiskResultsTableProps) {
  ...
  return (
    <div id="risk-report" className="bg-white rounded-lg border border-blue-100 shadow-sm p-4 md:p-6 max-w-[1400px] mx-auto overflow-x-auto print:shadow-none print:border-0 print:p-0 print:max-w-none print:rounded-none">
      {/* Header Table */}
      <table className="w-full border-collapse border border-blue-200 mb-0">
        <tbody>
          <tr>
            <td className={`${th} w-[80px]`}>평가직</td>
            <td className={`${td} w-[120px]`}>{formInfo.assessRole || "-"}</td>
            <td rowSpan={3} className="border border-blue-200 text-center align-middle px-4 min-w-[200px]">
          ...
          <tr>
            ...
            <td rowSpan={2} className="border border-blue-200 p-0 w-[140px]">
              <div className="flex h-full">
                <div className="flex-1 text-center py-2 bg-slate-100 border-r border-blue-200">
      ...
      {/* Main Results Table */}
      <table className="w-full border-collapse border border-blue-200 -mt-px">
        <thead>
          <tr>
        ...
        <tbody>
          {results.map((r, i) => {
            ...
            return (
              <tr key={i} className="hover:bg-blue-50/40 print:hover:bg-transparent">
      ...
      <table className="w-full border-collapse border border-blue-200 -mt-px">
        ...
        <tbody>
          <tr>
            <td className="border border-blue-200 p-0 align-top">
            ...
            <td className="border border-blue-200 p-0 align-top">
              <table className="w-full border-collapse">
                <thead>
                  <tr>
                    <th className="border border-blue-100 py-1 px-2 text-[10px] font-bold bg-[hsl(214_90%_96%)] w-1/6">이름</th>
                    <th className="border border-blue-100 py-1 px-2 text-[10px] font-bold bg-[hsl(214_90%_96%)] w-1/6">서명</th>
                    <th className="border border-blue-100 py-1 px-2 text-[10px] font-bold bg-[hsl(214_90%_96%)] w-1/6">이름</th>
                    <th className="border border-blue-100 py-1 px-2 text-[10px] font-bold bg-[hsl(214_90%_96%)] w-1/6">서명</th>
                    <th className="border border-blue-100 py-1 px-2 text-[10px] font-bold bg-[hsl(214_90%_96%)] w-1/6">이름</th>
                    <th className="border border-blue-100 py-1 px-2 text-[10px] font-bold bg-[hsl(214_90%_96%)] w-1/6">서명</th>
                ...
                <tbody>
                  {Array.from({ length: 6 }).map((_, i) => (
                    <tr key={i}>
                      <td className="border border-blue-100 py-2 px-2 text-xs">&nbsp;</td>
                      <td className="border border-blue-100 py-2 px-2 text-xs">&nbsp;</td>
                      <td className="border border-blue-100 py-2 px-2 text-xs">&nbsp;</td>
                      <td className="border border-blue-100 py-2 px-2 text-xs">&nbsp;</td>
                      <td className="border border-blue-100 py-2 px-2 text-xs">&nbsp;</td>
